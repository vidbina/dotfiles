...
(defun vidbina/init-eglot-pyright-config (input-project-dir input-venv-subdir)
  "Stub a pyrightconfig.json file for a project"
  (interactive "DProject root dir: \nDProject venv dir:")

  (let* ((project-dir (file-truename input-project-dir))
         (venv-dir (file-name-base (directory-file-name "~/hi/nice/test/"))))

    (message (format "🐍 Writing config for %s with venv in %s" project-dir venv-dir))
    (with-temp-buffer
      (insert "\nNext line is autogenerated! Modify file to make this a valid setup.\n")
      (insert (json-serialize (list :venvPath project-dir
                                    :venv venv-dir)))
